...
const toPointState = (point) => ({
  ...
  temperature: normalizeOptionalString(point?.temperature),
...
export const createMultipointEditorStateFromConfig = (config) => {
  ...
  return {
    ...
    points: points.length ? points : [{ number: "", name: "", temperature: "", itemizationRows: [] }],
  };
};
...
export const buildMultipointConfigFromEditorState = (state) => {
  ...
  config.metering_points = points.map((point) => {
    ...
    const temperature = normalizeOptionalString(point?.temperature);
